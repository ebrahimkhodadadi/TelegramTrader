...
def parse_message(message):
    try:
        ...
        return actionType, firstPrice, secondPrice, takeProfit, stopLoss
...
def GetTakeProfit(message):
    ...
    for sentence in sentences:
        ...
        # Extract TP numbers
        tp_match = re.search(r'tp\s*(?:\d*\s*:\s*)?(\d+\.\d+)', sentence, re.IGNORECASE)
        if not tp_match:
            tp_match = re.search(r'tp\s*:\s*(\d+\.?\d*)', sentence, re.IGNORECASE)
        if not tp_match:
            tp_match = re.search(r'tp1\s*:\s*(\d+\.?\d*)', sentence, re.IGNORECASE)
        if not tp_match:
            tp_match = re.search(r'tp\s*[-:]\s*(\d+\.\d+|\d+)', message, re.IGNORECASE)
        if tp_match:
...
def GetStopLoss(message):
    ...
    for sentence in sentences:
        ...
        sl_match = re.search(r'sl\s*:\s*(\d+\.\d+)', sentence, re.IGNORECASE)
        if not sl_match:
            sl_match = re.search(r'sl\s*:\s*(\d+\.?\d*)', sentence, re.IGNORECASE)
        if not sl_match:
            sl_match = re.search(r'STOP LOSS\s*:\s*(\d+\.?\d*)', sentence, re.IGNORECASE)
        if not sl_match:
